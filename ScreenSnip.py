...
class screenRegionPromptWidget(QMainWindow):
    ...
    def paintEvent(self, event):
        ...
        if self.mouseDownPoint is None:  # Fill the whole screen with white
            painter.fillRect(
                QRect(0, 0, self.desktop['width'], self.desktop['height']), backgroundBrush)
        else:
            '''
            ...
            # Region:
            region = self.regionFromTwoPoints(
                self.mouseDownPoint, self.mouseCurrentPoint)
            region['right'] = region['left'] + region['width']
            region['bottom'] = region['top'] + region['height']
            # Rect (1)
            painter.fillRect(
                QRect(0, 0, region['left'], self.desktop['height']), backgroundBrush)
            # Rect (2)
            painter.fillRect(
                QRect(region['left'], 0, region['width'], region['top']), backgroundBrush)
            # Rect (3)
            painter.fillRect(QRect(region['left'] + region["width"], 0, self.desktop['width'] -
                             region['right'], self.desktop['height']), backgroundBrush)
            # Rect (4)
            painter.fillRect(QRect(region['left'], region['bottom'], region['width'],
                             self.desktop['height'] - region['bottom']), backgroundBrush)

            # Now draw a red outline
            painter.setPen(QPen(QColor(255, 0, 0, 255), 1, Qt.SolidLine))
            painter.drawRect(region['left'], region['top'],
    ...
    def complete(self):
        if self.active and self.mouseDownPoint is not None and self.mouseUpPoint is not None:
            region = self.regionFromTwoPoints(
                self.mouseDownPoint, self.mouseUpPoint)
...
class outputWindowWidget(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.setWindowTitle("Output from BardAI")
        ...
        self.statusLabel = QLabel(
            "Scanning image for text...", self, objectName="statusLabel")
    ...
    def ocrStatusChange(self, id, status, data):
        if status == OCRSTATUS_BEGIN:
            ...
            self.statusLabel.setText(
                "Scanning image for (%s) text..." % str(language))
            ...
            err = data
            self.statusLabel.setText(
                "Processing timed out. Did you select the right language?")
            self.ocrResult.setPlaceholderText(
                "Timeout error... Make sure you have the right language selected, and your image is clearly legible!")
        elif status == OCRSTATUS_ERROR:
            err = data
            self.statusLabel.setText(
                "Unknown Error [%s]. Try reinstalling?" % str(err))
            self.ocrResult.setPlaceholderText(
...
supportedOCRScripts = [
    {"code": "script/Latin", "name": "Latin", "alphabet": "abcdefg",
        "examples": ["English", "French", "Spanish"]},
    {"code": "script/Cyrillic", "name": "Cyrillic", "alphabet": "абвгдеж",
        "examples": ["Russian", "Ukrainian", "Bulgarian"]},
    {"code": "script/Arabic", "name": "Arabic", "alphabet": "وهنملك",
        "examples": ["Iraqi", "Egyptian", "Moroccan"]},
]
...
class mainWindowWidget(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.screenSnipButton = QPushButton(
            "NEW", self.topbarItems, objectName="screenSnipButton")
        ...
        self.openImageButton = QPushButton(
            "OPEN", self.topbarItems, objectName="openImageButton")
        ...
        self.OCRLanguageSelector = QComboBox(
            self.topbarItems, objectName="ocrLanguageSelector")
        ...
        for lang in supportedOCRLanguages:
            self.OCRLanguageSelector.addItem(
                "%s (%s)" % (lang['name'], lang['local']))
        self.OCRLanguageSelector.insertSeparator(len(supportedOCRLanguages))
        for lang in supportedOCRScripts:
            self.OCRLanguageSelector.addItem(
                "%s Script (%s)" % (lang['name'], lang['alphabet']))
        self.OCRLanguageSelector.currentIndexChanged.connect(
            self.OCRLangChange)
    ...
    def openImagePressed(self):
        ...
        (fname, x) = QFileDialog.getOpenFileName(
            self, dialogTitle, openInDirectory, acceptedFiles)
        if x == '':
            return
        else:
            img = None
    ...
    def startOCR(self, image, id, language):
        ...
        try:
            text = getTextFromImg(image, timeout=120,
    ...
    def newOCR(self):
        ...
        if self.currentScanID == 1:  # the first time, setup our window
            ...
            self.topbarItems.setFixedSize(
                3 + 100 + 3 + 100 + 3 + 200 + 3, 50 - 6)
        ...
        if self.currentOCRSourceLanguageIndex < len(supportedOCRLanguages):
            ...
            language = supportedOCRScripts[self.currentOCRSourceLanguageIndex - len(
                supportedOCRLanguages) - 1]

        # show image
        h, w, ch = self.image_source.shape
        qimg = QImage(self.image_source.data.tobytes(),
                      w, h, ch * w, QImage.Format_RGB888)
        ...
        self.outputWindow.ocrStatusChangeSignal.emit(
            self.currentScanID, OCRSTATUS_BEGIN, language['name'])
        threading.Thread(target=self.startOCR, args=[
                         self.image_source, self.currentScanID, language]).start()
